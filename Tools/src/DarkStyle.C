...
void SetDarkSHINEStyle() {
  ...
  if (DarkSHINEStyle == nullptr)
    DarkSHINEStyle = DarkStyle();
...
TStyle *DarkStyle() {
  ...
  // darkStyle->SetFillColor(icol); // don't use: white fill color for *all*
  // objects
  ...
  darkStyle->SetLabelSize(tsize_i, "x");
  darkStyle->SetTitleSize(tsize, "x");
  darkStyle->SetLabelSize(tsize_i, "y");
  darkStyle->SetTitleSize(tsize, "y");
  darkStyle->SetLabelSize(tsize_i, "z");
  darkStyle->SetTitleSize(tsize, "z");

  darkStyle->SetNdivisions(510, "x");
  darkStyle->SetNdivisions(510, "y");
  ...
  darkStyle->SetLegendTextSize(0.025);
