...
void GblFitter::processTrackWithRep(Track* trk, const AbsTrackRep* rep, bool resortHits)
{
  ...
  for (unsigned int iIter = 0; iIter < m_externalIterations; iIter++) {
    ...
    std::vector<genfit_gbl::GblPoint> points = collectGblPoints(trk, rep);
    ...
    genfit_gbl::GblTrajectory traj(points, gblfs->hasCurvature());
...
std::vector<genfit_gbl::GblPoint> GblFitter::collectGblPoints(genfit::Track* trk, const genfit::AbsTrackRep* rep) {
  //TODO store collected points in in fit status? need streamer for GblPoint (or something like that)
  std::vector<genfit_gbl::GblPoint> thePoints;
  thePoints.clear();
...
void GblFitter::updateGblInfo(genfit_gbl::GblTrajectory& traj, genfit::Track* trk, const genfit::AbsTrackRep* rep) {
  //FIXME
  if (!traj.isValid())
    return;
