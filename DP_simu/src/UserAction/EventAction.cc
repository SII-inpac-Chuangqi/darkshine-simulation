...
{
    if ( frootMng->GetifFilter_Process() && ( !frootMng->GetFilter_Process_Result() || !frootMng->Filter_Process_EndofEvent() ) ) {
        frootMng->initialize();
        return;
    }
    if ( frootMng->GetifFilter_Particle() && !frootMng->Filter_Particle_EndofEvent() ) {
        frootMng->initialize();
        return;
    }
    if (event->IsAborted()) {
