...
// evenly along the track segment and uniformly into 4pi.
{
  ...
  if (verboseLevel > 0)  {G4cout << "No Parameter of Scint " << G4endl;aMaterialPropertiesTable->DumpTable();}
    return G4VRestDiscreteProcess::PostStepDoIt(aTrack, aStep);
    }
  ...
  if (MeanNumberOfPhotonsDecimated > 10.)
  {
    ...
    fNumPhotonsLUT = G4int(G4RandGauss::shoot(MeanNumberOfPhotonsDecimated, sigma) + 0.5);
  }
  ...
  for (G4int scnt = 1; scnt <= nscnt; scnt++)
  {
    ...
    { //
      if (nscnt == 1)
      { // only faat or slow component of sci. light
        if (Fast_Intensity)
        {
      ...
      { //fast + slow components: add fast
        G4double yieldRatio = aMaterialPropertiesTable->GetConstProperty(kYIELDRATIO);
        if (fExcitationRatio == 1.0 || fExcitationRatio == 0.0) //by default, this ratio is not set then taken yield ratio from material
        {
          Num = G4int(std::min(yieldRatio, 1.0) * fNumPhotons);
        }
        else
        {
          Num = G4int(std::min(fExcitationRatio, 1.0) * fNumPhotons); //if set exctition ratio, use this (for both )
        }
        ScintillationTime = aMaterialPropertiesTable->GetConstProperty(kFASTTIMECONSTANT);
        if (fFiniteRiseTime)
        {
    ...
    { //generate photons
      ...
      // We not use the real photons anymore in DSimu since the huge channel number.
      /*
      if (fCreatePhotons)
      {
        // Generate real photon:
        ...
        //Now push AUX information, only in DEV mode
        //in dev mode always save AUX, even not in bound, just save 0;
        if (fAddAUX)
        {
          OpticalTrackData *AUX = new OpticalTrackData();
          //save only calobound_good? or save default value?
          AUX->SetDepth(depth); //if not in the calobound_good region, then use <-1000
          AUX->SetCosTheta(local_cosTheta);
          AUX->SetCreationE(sampledEnergy);
          AUX->SetCreationT(aSecondaryTime);
          AUX->SetT0(t0);
          //aSecondaryPosition can also be saved... for further usage
          aSecondaryTrack->SetAuxiliaryTrackInformation(0, AUX); //hard-coded optical information saved to idx=-1
        }

        if (fScintillationTrackInfo)
          aSecondaryTrack->SetUserInformation(new G4ScintillationTrackInformation(ScintillationType));

        aParticleChange.AddSecondary(aSecondaryTrack);
      }
      */
      //fill genhit, no need anymore
      /*
      if (fSaveGen)
      {
        OpticalHit *newGenHit = new OpticalHit(copyNum);
        newGenHit->BuildGENHit(depth, local_cosTheta, aSecondaryTime, sampledEnergy, aSecondaryPosition,
                               fScintillation);
        newGenHit->SetT0(t0);
        fRootMgr->FillOpticalGen(newGenHit, cIn);
        delete newGenHit;
      }
      */
      //////////////////////////////////////////////////////
      ...
      if (!skipLookupHit && lookupTable != nullptr && lookupTable->isReady()) //for safe, check LUT
      {
        ...
        OpticalHit *newHit = new OpticalHit(copyNum);
        LUT_counter++;
        newHit->BuildLUTHit(*lookupTable, depth, local_cosTheta, aSecondaryTime, sampledEnergy, aSecondaryPosition, rand1, rand2, fScintillation);
        // newHit->SetT0(t0);
